...
const InventoryPanel = ({ player, onEquipItem, onDestroyItem }) => {
  ...
  // BẢNG SO SÁNH – ĐÃ FIX HOÀN TOÀN (không nhân đôi Two-Handed nữa)
  const calculateDiffs = (item) => {
    ...
    // =================================================================
    // CASE 1: Item đang xem là Two-Handed → so với cả 2 slot hiện tại
    // =================================================================
    if (isTwoHanded) {
      const w1 = player.equipment?.weapon1;
      const w2 = player.equipment?.weapon2;

      // Nếu không có vũ khí nào đang equip → không cần bảng so sánh
      if (!w1 && !w2) return null;

      // Tìm các item cũ DUY NHẤT (tránh nhân đôi nếu là Two-Handed)
      const oldItems = new Set();
      if (w1) oldItems.add(w1);
      if (w2 && w2.id !== w1?.id) oldItems.add(w2); // chỉ add nếu khác id
      ...
      // Các stat ngoài danh sách ưu tiên
      [
        ...new Set([...Object.keys(newStats), ...Object.keys(oldTotalStats)]),
      ].forEach((stat) => {
    ...
    // =================================================================
    // CASE 2: Item là vũ khí 1 tay → so từng slot riêng biệt
    // =================================================================
    else if (item.slot.includes("weapon")) {
    ...
    // =================================================================
    // CASE 3: Các món khác (áo, nhẫn, giày...) → giữ nguyên logic cũ
    // =================================================================
    else {
      for (const slot of getAvailableSlots(item)) {
        const equipped = player.equipment?.[slot];
            ...
            <div className="flex gap-3 py-2 min-w-max ml-1 mr-1">
              {inventoryItems.map((item) => (
                ...
                >
                  ...
                  />
                  {item.type === "twoHanded" && (
                    <div className="absolute bottom-0 right-0 bg-black/70 px-1.5 py-0.5 text-[10px] font-bold text-white rounded-tl">
                      2H
                    </div>
                  )}
                </button>
      ...
      {isMobile && selectedItem && (
        <div className="fixed inset-x-0 bottom-0 z-1003 flex flex-col gap-3 p-4 bg-gradient-to-t from-black/95 via-black/70 to-transparent">
          {diffPanels?.map((panel, idx) => (
            <div
              key={idx}
              className="bg-gray-950 border-2 border-cyan-600 rounded-lg p-4 shadow-2xl animate-in slide-in-from-bottom"
            >
          ...
          {diffPanels && (
            ...
            >
              {diffPanels.map((panel, idx) => (
                <div
                  key={idx}
                  className="bg-gray-950 border-2 border-cyan-600 rounded-lg p-4 shadow-2xl"
                >
                  <div className="text-cyan-400 text-xs font-bold mb-2 border-b border-cyan-900 pb-1">
                    {panel.label}
                  </div>
